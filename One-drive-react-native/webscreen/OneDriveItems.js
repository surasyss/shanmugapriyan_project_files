    ...
    this.props.route.params.chooseItems = this.props.route.params.chooseItems.filter(function(person) { 
      ...
      try{
        ...
        let file_type = each_item['file']['mimeType'].split('/')[0];
          ...
          if(file_type === 'image'){
            this.props.navigation.navigate('ShowFile', each_item);
          }else if(file_type === 'application'){
            alert(" Extention not avilable ...");
          }else if(file_type === 'text'){
            alert(" Extention not avilable ...");
          }else if(file_type === 'video'){
            this.props.navigation.navigate('VideoFile', each_item);
          }else{
            alert(" Extention not avilable ...");
          }
...
//   const newData = all_data_list.filter(item => {
...
// deSelectItems = () => console.log("----- deselected -------");
...
async each_folder_access(item, access_key){
  ...
  var _urlFiles = `https://graph.microsoft.com/v1.0/me/drive/items/${item.id}/children`;
    ...
    if (each_folder_list){
      ...
      this.props.navigation.navigate('FolderInside', this.props.route.params);
    }
  ...
  render() {
    ...
    // for (let k of all_files) {
    //   all_data_list.push(k);
    // }
