...
class LineChart extends AbstractChart {
  ...
  renderDots = (config) => {
    ...
    data.forEach((dataset) => {
      ...
      dataset.data.forEach((x, i) => {
        ...
        const cy = ((baseHeight - this.calcHeight(x, datas, height)) / 4) * 3
          + paddingTop;
        const onPress = () => {
  ...
  renderScrollableDot = (config) => {
    ...
    scrollableDotHorizontalOffset.addListener((value) => {
      ...
      if (index >= data[0].data.length - 1) {
        ...
        if (prev > base) {
          const rest = prev - base;
          this.label.current.setNativeProps({
            text: `${Math.floor(base + percent * rest)}`
          });
        } else {
          const rest = base - prev;
          this.label.current.setNativeProps({
            text: `${Math.floor(base - percent * rest)}`
          });
        }
        ...
        percent = 1 - percent;
        if (next > base) {
          const rest = next - base;
          this.label.current.setNativeProps({
            text: `${Math.floor(base + percent * rest)}`
          });
        } else {
          const rest = base - next;
          this.label.current.setNativeProps({
            text: `${Math.floor(base - percent * rest)}`
          });
        }
    ...
    return config.data.map((dataset, index) => (
      <Polygon
        ...
        points={
            `${dataset.data
              .map((d, i) => {
                const x = paddingRight
                  + (i * (width - paddingRight)) / dataset.data.length;
                const y = ((baseHeight - this.calcHeight(d, datas, height)) / 4) * 3
                  + paddingTop;
                return `${x},${y}`;
              })
              .join(' ')
  ...
  renderBezierLine = (config) => config.data.map((dataset, index) => {
  ...
  renderBezierShadow = (config) => {
    ...
    return data.map((dataset, index) => {
      const d = `${this.getBezierLinePoints(dataset, config)
      } L${paddingRight
          + ((width - paddingRight) / dataset.data.length)
            * (dataset.data.length - 1)},${(height / 4) * 3
  ...
  render() {
    ...
    const legendOffset = this.props.data.legend ? height * 0.05 : 0;
