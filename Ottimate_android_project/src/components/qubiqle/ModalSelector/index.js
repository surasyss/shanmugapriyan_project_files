...
export default class ModalSelector extends React.Component {
  constructor(props) {
    super(props);
    const selectedItem = this.validateSelectedKey(props.selectedKey);
    this.state = {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevProps.selectedKey !== this.props.selectedKey || prevProps.data !== this.props.data) {
      const selectedItem = this.validateSelectedKey(this.props.selectedKey);
  ...
  validateSelectedKey = (key) => {
    const selectedItem = this.props.data.filter((item) => this.props.keyExtractor(item) === key);
    const selectedLabel = selectedItem.length > 0 ? this.props.labelExtractor(selectedItem[0]) : this.props.initValue;
    const selectedKey = selectedItem.length > 0 ? key : undefined;
  ...
  renderChildren = () => {
    ...
    const initSelectStyle = this.props.initValue === this.state.selected
