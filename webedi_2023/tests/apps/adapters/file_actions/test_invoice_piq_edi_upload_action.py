...
from apps.adapters.file_actions import InvoiceEDIPIQUploadAction, SkipProcessing
...
def test__skip_s3_upload():
    class Action(InvoiceEDIPIQUploadAction):
...
def test__create_piq_container__create_invoice_being_called_with_required_parameters(
    ...
    assert arguments.get("is_edi")
    assert "job" in arguments.keys()


@override_settings(DISCOVERED_FILE_PIQ_CREATE_DOC=True)
def test_invoice_edi_piq_upload_action__get_payload():
    ...
    fda = df.discovery_action
    edi_type = EDIType(fda.edi_parser_code)
    ...
    action = InvoiceEDIPIQUploadAction(discovered_file=df)
