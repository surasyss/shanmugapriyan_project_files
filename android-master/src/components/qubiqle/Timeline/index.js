...
const defaultCircleColor = Colors.history;
const defaultLineWidth = 0.5;
const defaultLineColor = Colors.history;
...
export default class Timeline extends PureComponent {
  ...
  _renderItem({ item, index }) {
    ...
    switch (this.props.columnFormat) {
      ...
      case 'single-column-right':
        ...
        break;
      case 'two-column':
        content = (item.position && item.position == 'right') || (!item.position && index % 2 == 0) ? (
          <View style={[styles.rowContainer, this.props.rowContainerStyle]}>
            {this.renderTime(item, index)}
            {this.renderEvent(item, index)}
            {this.renderCircle(item, index)}
  ...
  _renderEvent(rowData, rowID) {
    ...
    const lineColor = rowData.lineColor ? rowData.lineColor : this.props.lineColor;
    let opStyle = null;
  ...
  _renderDetail(rowData, rowID) {
    ...
    return (
      <View style={styles.container}>
        <Text style={[styles.title, this.props.titleStyle, rowData.titleStyle, {color : rowData.color}]}>
          {rowData.title}
        </Text>
        {description}
      </View>
  ...
  _renderCircle(rowData, rowID) {
    ...
    if (rowData.color && rowData.color !== Colors.gray) {
      circleStyle.backgroundColor = rowData.color;
    }

    let innerCircle = null;
    switch (this.props.innerCircle) {
      case 'icon':
        const iconDefault = rowData.iconDefault ? rowData.iconDefault : this.props.iconDefault;
