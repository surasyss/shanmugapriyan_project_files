...
class CreditRequestDetail extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      filteredUsers: []
  ...
  onTabSelection = (index) => {
    const { creditRequest, setCurrentTab } = this.props;
    if (index.i === 0) sendMixpanelEvent(MixpanelEvents.CREDIT_REQUEST_ITEMS_OPENED, { creditRequest });
    else if (index.i === 1) sendMixpanelEvent(MixpanelEvents.CREDIT_REQUEST_GL_SPLITS_OPENED, { creditRequest });
    else if (index.i === 2) sendMixpanelEvent(MixpanelEvents.CREDIT_REQUEST_IMAGES_OPENED, { creditRequest });
    else if (index.i === 3) sendMixpanelEvent(MixpanelEvents.CREDIT_REQUEST_HISTORY_OPENED, { creditRequest });
    if (setCurrentTab) {
  ...
  renderHeader() {
    const { creditRequest, restaurants } = this.props;
    let vendor_name = '';
    const {
      vendor_obj, invoice_number, date, total_amount, is_vendor_supplied_invoice, is_flagged, notes
    } = creditRequest;
    ...
    if (restaurants) {
      restaurants.forEach((restaurant) => {
        if (restaurant.id === creditRequest.restaurant) {
    ...
    return (
      <View style={styles.header}>
        ...
        <View style={[styles.headerItem, styles.noBorder]}>
          <Text style={[styles.headerHeading, notes ? {} : styles.missing]}>Memo</Text>
          <Text style={[styles.headerValue, notes ? {} : styles.missingValue]}>{notes || ''}</Text>
        </View>
      </View>
    );
  }
  ...
  renderFlagDialog() {
    const {
      isShowFlagDialog, showFlagAlert, flagText, setFlagText, creditRequest, addCreditRequestFlag, resolveCreditRequestFlag
    } = this.props;
    const { is_flagged } = creditRequest;
    ...
    return (
      ...
      >
        <View style={styles.modalContainer}>
          <Text style={styles.modalHeading}>{is_flagged ? 'Resolve Credit Request' : 'Flag Credit Request'}</Text>
          <MentionsTextInput
            trigger="@"
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                if (flagText && flagText.length) {
                  ...
                  if (is_flagged) {
                    resolveCreditRequestFlag(creditRequest.id, formattedText);
                    showFlagAlert(false);
                  } else {
                    addCreditRequestFlag(creditRequest.id, formattedText);
                    showFlagAlert(false);
                  }
                }
              }}
  ...
  renderLineItems() {
    ...
    const { has_manual_splits, is_ap_lite, has_line_items_loaded } = creditRequest;
    let { line_items } = creditRequest;
  ...
  renderGlSplits() {
    const {
      creditRequest, expandGlSplits
    } = this.props;
    const { has_gl_splits_loaded } = creditRequest;
    let { gl_splits } = creditRequest;
  ...
  renderImages() {
    const {
      creditRequest, goToInvoiceImage
    } = this.props;
    const { has_images_loaded } = creditRequest;
    let { images } = creditRequest;
    if (!images) images = [];
  ...
  renderHistory() {
    ...
    const { has_history_loaded } = creditRequest;
    let { history } = creditRequest;
    if (!history) history = [];
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        {creditRequest ? this.renderHeader() : null}
        {creditRequest ? this.renderTabs() : null}
        {creditRequest ? this.renderFlagDialog() : null}
        {creditRequest ? this.renderLoadingDialog() : null}
      </View>
    );
  }
}
